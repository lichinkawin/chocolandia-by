...
4. Картинки товаров в **`site/NEW/`** в Git не хранятся (см. `.gitignore`); на прод **загрузите папку `NEW/` целиком** через файловый менеджер/FTP. Имена файлов в таблице и на диске должны совпадать **в том же регистре**, что важно на Linux-хостинге.
